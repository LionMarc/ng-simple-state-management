...
import { NgssmDataActionType, NgssmSetDataSourceParameterAction, NgssmSetDataSourceParameterValidityAction, NgssmUpdateDataSourceParameterAction } from '../actions';
...
describe('DataSourceParameterReducer', () => {
    ...
    describe(`when processing action of type '${NgssmDataActionType.setDataSourceParameter}'`, () => {
        describe('when parameter is not set in action', () => {
            ...
            it(`should set source value valueOutdated to true`, () => {
                const updatedState = reducer.updateState(state, action);
                expect(selectNgssmDataState(updatedState).dataSourceValues['data-providers']?.valueOutdated).toBe(true);
            });
        ...
        describe('when parameter is set in action', () => {
            ...
            it(`should not set source value valueOutdated to true when doNotMarkParameterAsModified is set to true`, () => {
                const updatedState = reducer.updateState(state, new NgssmSetDataSourceParameterAction('data-providers', 'new parameter', undefined, true));
                expect(selectNgssmDataState(updatedState).dataSourceValues['data-providers']?.valueOutdated).toBe(false);
            });
        });
    ...
    describe(`when processing action of type '${NgssmDataActionType.updateDataSourceParameter}'`, () => {
        ...
        it(`should not set source value valueOutdated to true when doNotMarkParameterAsModified is set to true`, () => {
            const updatedState = reducer.updateState(state, new NgssmUpdateDataSourceParameterAction('data-providers', {
                onlyLast: false,
                description: 'something'
            }, true));
            expect(selectNgssmDataState(updatedState).dataSourceValues['data-providers']?.valueOutdated).toBe(false);
