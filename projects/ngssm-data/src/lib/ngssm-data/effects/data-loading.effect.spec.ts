...
import { NgssmDataActionType, NgssmLoadDataSourceAdditionalPropertyValueAction, NgssmLoadDataSourceValueAction, NgssmSetDataSourceAdditionalPropertyValueAction, NgssmSetDataSourceValueAction } from '../actions';
...
describe('DataLoadingEffect', () => {
    ...
    const dataProvidersAdditionalPropertyLoadingFunc = vi.fn((state: State, dataSourceKey, property: string) => of({ label: property, dataSourceKey }));
    const dataProvidersLoadingFailsFunc = vi.fn(() => of(['test'])).mockReturnValue(throwError(() => ({
        title: 'bad call'
    })));
    const dataProvidersAdditionalPropertyLoadingFailsFunc = vi.fn((state: State, dataSourceKey, property: string) => of({ label: property })).mockReturnValue(throwError(() => ({
        title: 'bad call'
    })));
    const managersLoadingFunc = vi.fn(() => {
        const store = inject(Store);
        return of([store.state()['testing']]);
    });
    const managersAdditionalPropertyLoadingFunc = vi.fn((state: State, dataSourceKey, property: string) => {
        const store = inject(Store);
        return of({ state: store.state()['testing'], label: property, key: dataSourceKey });
    });
    const managersLoadingFailsFunc = vi.fn(() => {
        return of(['main']);
    }).mockReturnValue(throwError(() => ({
        title: 'bad call'
    })));
    const managersAdditionalPropertyLoadingFailsFunc = vi.fn((state: State, dataSourceKey, property: string) => {
        const store = inject(Store);
        return throwError(() => ({
            title: 'bad call',
            property,
    ...
    describe(`when processing action of type '${NgssmDataActionType.loadDataSourceValue}'`, () => {
        ...
        it(`should log an information when data source value is not in '${NgssmDataSourceValueStatus.loading}' status`, () => {
            ...
            expect(logger.information).toHaveBeenCalledWith(`Data source value for 'data-loaded' is not in '${NgssmDataSourceValueStatus.loading}' status: '${NgssmDataSourceValueStatus.loaded}'`);
        });
        ...
        describe(`when loading function contains no inject call`, () => {
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceValue}' with status '${NgssmDataSourceValueStatus.loaded}' when loading succeeds`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceValueAction('data-providers', NgssmDataSourceValueStatus.loaded, ['test']));
            });
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceValue}' with status '${NgssmDataSourceValueStatus.error}' when loading fails`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceValueAction('data-providers-ko', NgssmDataSourceValueStatus.error, undefined, {
                    title: 'bad call'
        ...
        describe(`when loading function contains an inject call`, () => {
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceValue}' with status '${NgssmDataSourceValueStatus.loaded}' when loading succeeds`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceValueAction('managers', NgssmDataSourceValueStatus.loaded, [
                    {
                        description: 'test'
                    }
                ]));
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceValue}' with status '${NgssmDataSourceValueStatus.error}' when loading fails`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceValueAction('managers-ko', NgssmDataSourceValueStatus.error, undefined, {
                    title: 'bad call'
                } as unknown as HttpErrorResponse));
            });
        });
    ...
    describe(`when processing action of type '${NgssmDataActionType.loadDataSourceAdditionalPropertyValue}'`, () => {
        ...
        it(`should log an information when data source property value is not in '${NgssmDataSourceValueStatus.loading}' status`, () => {
            ...
            expect(logger.information).toHaveBeenCalledWith(`Data source additional property value for 'data-loaded' and property 'my-prop' is not in '${NgssmDataSourceValueStatus.loading}' status: '${NgssmDataSourceValueStatus.loaded}'`);
        });
        ...
        describe(`when loading function contains no inject call`, () => {
            it(`should execute the loading function`, () => {
                ...
                expect(dataProvidersAdditionalPropertyLoadingFunc).toHaveBeenCalledWith(expect.any(Object), 'data-providers', 'my-prop');
            });
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceAdditionalPropertyValue}' with status '${NgssmDataSourceValueStatus.loaded}' when loading succeeds`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceAdditionalPropertyValueAction('data-providers', 'my-prop', NgssmDataSourceValueStatus.loaded, {
                    label: 'my-prop',
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceAdditionalPropertyValue}' with status '${NgssmDataSourceValueStatus.error}' when loading fails`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceAdditionalPropertyValueAction('data-providers-ko', 'my-prop', NgssmDataSourceValueStatus.error, undefined, undefined, {
                    title: 'bad call'
                } as unknown as HttpErrorResponse));
            });
        });
        ...
        describe(`when loading function contains an inject call`, () => {
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceAdditionalPropertyValue}' with status '${NgssmDataSourceValueStatus.loaded}' when loading succeeds`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceAdditionalPropertyValueAction('managers', 'my-prop', NgssmDataSourceValueStatus.loaded, {
                    state: {
                        description: 'test'
                    },
                    label: 'my-prop',
            ...
            it(`should dispatch an action of type '${NgssmDataActionType.setDataSourceAdditionalPropertyValue}' with status '${NgssmDataSourceValueStatus.error}' when loading fails`, () => {
                ...
                expect(store.dispatchAction).toHaveBeenCalledWith(new NgssmSetDataSourceAdditionalPropertyValueAction('managers-ko', 'my-prop', NgssmDataSourceValueStatus.error, undefined, undefined, {
                    title: 'bad call',
                    property: 'my-prop',
                    state: {
                        description: 'test'
