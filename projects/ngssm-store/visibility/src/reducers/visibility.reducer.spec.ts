...
describe('VisibilityReducer', () => {
  ...
  describe(`when processing action of type '${NgssmVisibilityActionType.showElement}'`, () => {
    ...
    it(`should set element as visible`, () => {
      ...
      expect(selectNgssmVisibilityState(updatedState).elements['testing']).toBeTrue();
    });
    ...
    it(`should set all elements of groups associated to the updated element as not visible`, () => {
      ...
      expect(selectNgssmVisibilityState(updatedState).elements['one']).toBeFalse();
      expect(selectNgssmVisibilityState(updatedState).elements['two']).toBeFalse();
      expect(selectNgssmVisibilityState(updatedState).elements['three']).toBeTrue();
    });
  });
});
